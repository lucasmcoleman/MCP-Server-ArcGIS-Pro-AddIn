...
| Two parameters or step outputs share a name | `create_model`/`update_model` **reject it** (added 2026-07-12) with `Duplicate name '<name>' in model definition: parameter/output names must be unique across the model` — rename one and retry. On a bridge build/deploy that predates this fix, there's no guard: the second write silently wins and every ref to that name resolves to the wrong (last) id |
...
| Model has Model Properties-level default environments | As of 2026-07-12, `update_model` preserves them (see the data-loss section below) — but that fix ships in the Add-In, so confirm the running bridge has actually been redeployed past this fix before trusting it; when in doubt, still prefer `set_parameter_default`/`set_step_parameter` |
...
- **Every parameter name and every step output name must be unique across the whole model.** `GenerateModelFiles`' `nameToId` map is a single shared namespace across inputs and step outputs. As of 2026-07-12, both write sites (`AtbxManager.cs`, the input-parameter loop and the step-output loop) check `nameToId` before writing and throw `Duplicate name '<name>' in model definition: parameter/output names must be unique across the model` — a structured `create_model`/`update_model` failure at write time, not a silent miswire. **This fix is Add-In-side**: a bridge that hasn't been redeployed past 2026-07-12 still has the old behavior — a second write with the same name silently overwrites the first, and every ref to that name from anywhere in the model resolves to the wrong (last) id, with no error.
...
`update_model` (via `GenerateModelFiles`) used to regenerate `tool.model` and `tool.content` **only** from the fields the simplified schema defines: `tool.model` got `version`/`updated`/`variables`/`processes`; `tool.content` got `type`/`displayname`/`description`/`app_ver`/`product`/`updated`/`params`. Anything Pro stores outside that shape — most importantly **model-level default environments** set via the Model Properties dialog — was silently dropped, with no warning.

**Fixed 2026-07-12:** `update_model` now reads the existing `tool.model`/`tool.content` from the zip before writing and clones them as the merge base, overwriting only the keys the generator owns (the same key lists as above; `params` is explicitly removed rather than left dangling when the new definition has zero exposed parameters). Every other root key Pro wrote — model-level environments included — now survives an update verbatim. `create_model` is unaffected (nothing to merge onto for a new model).

**This fix is Add-In-side and requires a redeploy to take effect.** It lives in `AddIn/APBridgeAddIn/ModelBuilder/AtbxManager.cs`; until the `.esriAddinX` bundle is rebuilt and redeployed (see **releasing-and-deploying**) and Pro restarted onto the new build, a live bridge still exhibits the old drop-on-update behavior described above. If you're not sure which build is currently loaded, treat it as unfixed: before running `update_model` on a model someone authored in Pro (not one the bridge created), check whether it has model-level environments set, and if it does, prefer the surgical primitives instead — `set_parameter_default` and `set_step_parameter`. Both route through `WriteAtbxAtomically` and mutate the existing file in place rather than regenerating it, so they never depended on this fix in the first place.
...
- Reusing a parameter or output name across steps because it "seemed fine" — since 2026-07-12, `create_model`/`update_model` reject this outright with a clear `Duplicate name` error; on a bridge predating that fix (Add-In not yet redeployed), it's still silent misrouting, not an error.
...
- Running `update_model` on a Pro-authored model without checking for model-level environments first, then wondering why a downstream tool that depended on an inherited environment starts failing — fixed 2026-07-12 (see the data-loss section above), but only on a bridge that's actually been redeployed past that fix.
