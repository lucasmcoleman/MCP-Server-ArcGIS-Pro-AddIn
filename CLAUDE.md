...
- **`AtbxManager.cs`** (~2.2K lines) owns ModelBuilder `.atbx` read/write. `.atbx` files are ZIPs containing per-tool folders (`{toolName}.tool/tool.model` + content + diagram); `WalkModel` parses one into a typed `ModelGraph` (variables + topo-sorted processes), `GenerateModelFiles` writes the JSON files from a simplified definition. Every `.atbx` write — bulk (`CreateModel`, `UpdateModel`) and surgical (`SetParameterDefault`, `SetStepParameter`) — routes through `WriteAtbxAtomically` (see "Things that bite").
...
- **`SanitizeGdbName`** translates ModelBuilder variable names into valid File Geodatabase FC names when deriving output paths (spaces → `_`, leading-digit prefix → `x_`). Since 2026-07-12 collisions get run-wide `_2`/`_3` suffixes (stable per original name, shared across nested-model recursion) — two names that sanitize identically no longer silently overwrite.
- **Runs are serialized process-wide** (2026-07-12): a `SemaphoreSlim(1,1)` gates RunModelCore, so a second concurrent `run_model`/`start_run_model` queues (its job status says it's waiting) instead of racing the single arcpy session. Steps with 2+ derived outputs are refined positionally from `IGPResult.Values` when counts align, else kept as placeholders with a loud warning. The `.pyt` 000464 retry also matches "schema lock" wordings, not just the literal code. If `SystemToolboxCatalog` self-disables (its Buffer sanity check fails, e.g. after a Pro upgrade changes `tool.content`'s shape), `SystemToolboxCatalog.DisabledReason` records why and the executor surfaces it in step messages instead of silently degrading to dense-packing.
...
The scheduled task + VBS launcher behind the HTTP/Copilot Studio deployment are documented in `docs/http-deployment.md` and reconstructible via `tools/Register-HttpServerTask.ps1` (the `MCP_AUTH_TOKEN` value lives only in the host's user-scope env var, never in the repo).

## Repo skills

Seven task-specific skills live in `.claude/skills/` (auto-discovered by Claude Code): `authoring-atbx-models`, `debugging-model-runs`, `adding-bridge-tools`, `releasing-and-deploying`, `operating-live-pro`, `validating-gis-outputs`, `sharing-the-http-bridge`. They carry the newest operational detail (error tables, checklists, deployed-vs-fixed caveats) and each states when NOT to use it — prefer them over re-deriving a procedure from this file.
...
**Parameter-interface fidelity (2026-07-10):** `tool.content`'s `params` object is Pro's authoritative public interface — its key order is the dialog/arcpy calling order and per-entry `type:"optional"` carries optionality (absent = Required); `tool.model`'s variables array is creation order and routinely disagrees on Pro-authored models. `describe_model` therefore emits `optional: true` per input, orders `inputs` by tool.content, marks tool.model variables that carry the Parameter flag but have NO tool.content entry `"exposed": false` (Pro-authored strays, e.g. auto-named `28` — they must NOT be promoted into the interface), and emits top-level `parameterOrder` (full interface order, derived outputs interleaved). `GenerateModelFiles` honors all four; definitions without the fields keep the old behavior. Agents editing definitions must preserve these fields verbatim — dropping them makes every parameter Required and re-derives the order (the pre-fix bug: ERROR 000735 cascades at run time). Round-trip fidelity assertions live in AtbxTests, including doctored Pro-authored-shape fixtures — extend them when touching either side. Two further writer guarantees landed 2026-07-12: duplicate parameter/step-output names are REJECTED with a clear error (they used to silently misroute every ref to the last-assigned id), and the update path merges into the existing `tool.model`/`tool.content` JSON so root keys outside the simplified schema (e.g. model-level default environments) survive `update_model` instead of being dropped.

## Testing

- **`dotnet run --project tests/AtbxTests -c Release`** — out-of-process round-trip suite (63 checks) for the ModelBuilder file layer (compiles AtbxManager + catalogs by source inclusion). Needs ArcGIS Pro *installed* (not running) for the SystemToolboxCatalog section, which auto-SKIPs cleanly when Pro is absent — that skip is what lets CI run the suite on hosted runners. Run after any AtbxManager/GpToolCatalog/SystemToolboxCatalog change.
- **CI** (`.github/workflows/ci.yml`) builds the MCP server and runs AtbxTests on every push/PR; `release.yml` runs the suite before publishing, so a broken file layer blocks a release. Neither builds the Add-In (needs Pro SDK assemblies; the release workflow's `build-addin` job is gated on a self-hosted runner that has never been registered — the `.esriAddinX` on releases is built and uploaded manually).
- **`./tools/Test-BridgeLive.ps1`** — ~44-check live smoke battery over the named pipe (Pro must be open, AFTER the Python warm-up window). Run after any Add-In deploy. It does NOT exercise run_model/create_model — the executor has no automated coverage; validate executor changes with the scratch-toolbox pattern against live Pro.
- **`./tools/Test-McpStdio.ps1`** — drives the MCP surface over stdio; defaults to the `publish/` exe that `.mcp.json` actually loads (pass `-ServerPath` to target a different build).
